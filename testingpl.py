    ...
    while queue:
        ...
        if(node.x==goalnode.x and node.y==goalnode.y):
            ...
            for neigh in node.neighbours:
                newg = node.g + ((node.x - neigh.x)**2 + (node.y - neigh.y)**2) ** 0.5
...
def PRMphase1(obs,gridmap):
    ...
    for sample in samples:
        findneighbours(gridmap,sample,samples,maxx/12,obs)
...
def PRMphase2(gridmap,obs,samples, goal,start):
    ...
    maxx=len(gridmap[0])
    tempsamples = samples
    findneighbours(gridmap,goal,tempsamples,maxx/12,obs)
    findneighbours(gridmap,start,tempsamples,maxx/12,obs)
